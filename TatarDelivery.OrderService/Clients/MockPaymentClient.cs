...
public class MockTinkoffPaymentClient : IPaymentClient
{
        ...
        ILogger<MockTinkoffPaymentClient> logger)
    {
        ...
        // 🔧 Читаем флаг: по умолчанию МОК, а не реальный HTTP
    ...
    public async Task<PaymentResponse> CreatePaymentAsync(CreatePaymentRequest request)
    {
        ...
        // 🎲 Если режим мока — возвращаем фейковый ответ без HTTP
        if (!_useRealHttp)
        {
            return GenerateMockPaymentResponse(request.OrderId);
        }

        // 🌐 Если включён реальный режим — пробуем сделать запрос
        try
        {
        ...
        {
            // ⚠️ Если сервис недоступен — падаем в мок, а не крашим приложение
    ...
    // 🎲 Генерация мок-ответа для CreatePayment
    private PaymentResponse GenerateMockPaymentResponse(string orderId)
    {
        // 90% успех, 10% отказ — чтобы тестировать retry-политики Polly
    ...
    // 🎲 Генерация мок-ответа для GetPaymentStatus
